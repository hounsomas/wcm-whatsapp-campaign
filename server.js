...
const db = new sqlite3.Database('wcm.db');
...
cron.schedule('* * * * *', () => {
  ...
  db.all(
    'SELECT * FROM campaigns WHERE scheduled_time <= ? AND status = "scheduled"',
    [now],
    (err, campaigns) => {
      if (err) {
        console.error('Erreur lors de la vérification des campagnes programmées:', err);
        return;
      }
      ...
      campaigns.forEach(campaign => {
        ...
        // Ici, vous intégreriez l'API WhatsApp Business réelle
...
// Démarrer le serveur
app.listen(PORT, () => {
  console.log(`🚀 Serveur WCM démarré sur le port ${PORT}`);
  console.log(`📱 Plateforme de campagnes WhatsApp prête à l'emploi`);
